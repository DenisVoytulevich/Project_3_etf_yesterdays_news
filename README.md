...
| Источник | Назначение |
|----------|------------|
| `DAILY_BRIEFING_ENABLED` | Fallback, если нет runtime-файла |
| `data/service_state.json` | Runtime-флаг из Рабочего кабинета микросервисов |

При `enabled: false`: автоотчёт в 09:00 не запускается; кнопка в боте отвечает «Функция отключена с Рабочего кабинета микросервисов.»
